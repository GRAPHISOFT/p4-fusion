...
int Main(int argc, char** argv)
{
	...
	Arguments::GetSingleton()->OptionalParameter("--mergeDeletes", "true", "When enabled, files that are deleted or moved on a target branch as part of an integration changelist are folded into the merge commit rather than being placed in a separate non-merge commit. "
	                                                                     "Only applies when exactly one source branch is involved; if multiple source branches target the same branch in one changelist, the deletes remain in their own commit.");
	...
	BranchSet branchSet(git,
	    P4API::ClientSpec.mapping,
	    depotPath,
	    branchNames,
	    mappings,
	    exclusions,
